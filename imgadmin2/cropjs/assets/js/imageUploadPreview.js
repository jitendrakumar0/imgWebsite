  ...
  if (input.files && input.files[0]) {
    ...
    reader.onload = function(e) {
      if (e.target.result) {
        ...
        var upperValue = parseFloat(aspct.split("/")[0]);
        var lowerValue = parseFloat(aspct.split("/")[1]);
        // create new image
        ...
        cropper = new Cropper(img, {
          ...
          aspectRatio: upperValue/lowerValue,
          // aspectRatio: 16/9.14,
          ...
          crop: function(e) {
            ...
            if (
              Math.round(e.detail.width) == 0 ||
              Math.round(e.detail.height) == 0
            ) {
              showImageSizes = 'auto';
            } else {
              showImageSizes =
                Math.round(e.detail.width) + 'X' + Math.round(e.detail.height);
            }
...
$('.custom-file-upload input').on('click', function() {
  ...
  if (c == 'true') {
    $('#' + d).children('#imageCroper' + d).remove();
    $('#' + d).append(
      ` <div class="modalCustom fade" id="imageCroper` +
        d +
        `">
                                ...
                                <div class="modalCustom-dialog modalCustom-fullscreen modalCustom-dialog-scrollable modalCustom-dialog-centered w-100">
                                    <div class="modalCustom-content">
                                        ...
                                        <div class="modalCustom-body text-dark">
                                            <div class="modal_85834">
                                                <!-- leftbox -->
                                                <div class="box-2 col-12 px-0 h-100">
                                                    <div id="result` +
        d +
        `" class="result h-100"></div>
                                                </div>
                                            </div>
                                        </div>
...
$('.custom-file-upload input').on('change', function() {
  ...
  if (c == 'true') {
    ...
    if (par.attr('data-zoom') == 'true') {
      $('#zoomInBtn')
        .append(`  <button type="button" id="zoomIN" onClick="zoomin()" class="btn zi-1 text-white fs-20 fw-bold text-uppercase p-0 h-30px d-flex align-items-center justify-content-center shadow-none">
                                                <i class="imgl img-search-plus"></i>
                                            </button>`);
      $('#zoomOutBtn')
        .append(`  <button type="button" id="zoomOut" onClick="zoomout()" class="btn zi-1 text-white fs-20 fw-bold text-uppercase p-0 h-30px d-flex align-items-center justify-content-center shadow-none">
                                                <i class="imgl img-search-minus"></i>
                                            </button>`);
    }
    if (par.attr('data-rotate') == 'true') {
      $('#rightCropperBtn')
        .append(`  <!--<button type="button" id="rotateR" onClick="rorateLeft()" class="btn zi-1 text-white fs-18 mx-3 fw-bold text-uppercase col-auto p-0 h-30px d-flex align-items-center justify-content-center shadow-none">
    ...
    if (par.attr('data-download') == 'true') {
      $('#dowloadPosition')
        .append(`  <a onClick="downloadBtn()" download="cropper.png" class="btn zi-1 text-white fs-20 fw-bold text-uppercase p-0 h-30px d-flex align-items-center justify-content-center shadow-none">
                                                <i class="imgl img-arrow-alt-to-bottom"></i>
                                            </a>`);
    }
    (result = document.querySelector('.result')), (imgSrc = null);
    (img_result = document.querySelector(
      '.img-result'
    )), (img_w = document.querySelector(
      '.img-w'
    )), (img_h = document.querySelector(
      '.img-h'
    ...
    // let checkimg = check(this);
    // console.log('jhsdgfasdh'+check(this));
    check(this);
    // var jitu = par.attr('bgChecker');
    // console.log(par.attr('bgChecker'));
    // if(jitu==1){
    //   colorCheck(this);
    // }
...
function colorCheck(input){
  ...
  reader.onload = function (e) {
	...
	var rgb = colorThief.getBackGroundColor(image);
	// console.log('background-color = '+rgb);
	rgb2hsl(rgb[0],rgb[1],rgb[2]);
...
function rgb2hsl(r, g, b) {
  ...
  if (l > 60) {
    ...
    closeCropper();
    alert("Image should not be in white background color");
  }
}
...
function check(input){
  ...
  reader.onload = function (e) {
    ...
    image.onload = function () {
      ...
      var type = $('#upload-button').attr('data-type');
      if(width2<1080 && type!='logo'){
        // console.log('check if');
        // return "1";
        closeCropper();
        alert("Image width should be more than 1080px");
      }

      if(width2<20 && type=='logo'){
        // console.log('check if');
        // return "1";
        closeCropper();
        alert("Image width should be more than 512px");
      }
...
function addEvents() {
  ...
  obj.x = imgSrc;
  obj.y = 1;
  obj.type = $('#upload-button').attr('data-type');
...
function downloadBtn() {
  ...
  if (par.attr('data-download') == 'true') {
    $('#downloadimages').hide();
    $('#waitForCanvasDownload')
      .append(` <a class="btn btn-dark border rounded-pill zi-1 text-white fs-10 fw-bold text-uppercase col-auto waitForCanvasDownload">
